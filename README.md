...
- 지속적인 Cron 작업이 필요합니다. (dependency가 있는 작업들도 존재합니다) 어떻게 작업들을 관리할까요?
	- ##### 주기적인 job을 권한, 주기에 맞게 cron포멧파일로 저장합니다.
	- ##### dependency가 생기는 job은 bash스크립트로 관리를 할 수가 있습니다.
	- ##### 프로그램실행에 따른 부수적인 출력제거 및 비동기 실행에 대해서는 " > /dev/null 2>&1 &" postfix로 관리가 가능합니다
- 처음 서버를 샀습니다. 어떤 보안적 조치를 먼저 하시겠습니까?
	- ##### OS보안업데이트 및 SW/package업데이트 진행합니다.
	- ##### 포트오픈 및 SSH/FTP 등 access point를 제한합니다.
	- ##### 유저생성 및 권한부여를 합니다. root 계정 접근을 최소화합니다.
	- ##### 상당복잡한 pw를 적용합니다.
	- ##### Restore point를 백업합니다.
- SSH로의 brute-force attack을 막기 위해서 어떤 조치를 취하고 싶으신가요?
	- ##### ssh포트를 22에서 다른 번호로 바꿉니다.
	- ##### iptables나 tcpwrapper로 룰을 적용하여 특정ip만 허용하거나 특정ip를 제한하면 됩니다.	
- 프로세스의 CPU 상태를 보기 위해 top을 했습니다. user,system,iowait중에 뭐를 제일
신경쓰시나요? 이상적인 프로그램이라면 어떻게 저 값들이 나오고 있어야 할까요?
	- ##### 가장신경쓰는 수치는 iowait입니다. iowait는 io부하가 심할 시 프로세스가 io를 기다리는데 많은 시간을 나타내는 수치입니다.
	- ##### 네트워크 밴드윗부하, SSD/HDD의 부하, 메모리부족, 메모리밴드윗부하 등을 의심할 수 있습니다.
	- ##### iowait가 높아질 시 서버를 최적활용하지 못하고 있기때문에 user / system 보다 더 중요합니다.
	- ##### 로직최적화, 하드웨어업그래이드, 시스템구조변경 등 해결방법이 있습니다.
- iowait이 높게 나왔다면, 내가 해야하는 조치는 무엇인가요? (돈으로 해결하는 방법과 소프트웨어로 해결하는 방법을 대답해주세요)
	- ##### SSH/HDD, L3/L4 switch, 메모리 업그래이드를 통해 어느정도 해결할 수 있습니다. 하드웨어를 업그래이드 시 돈으로 해결가능하지만 부하에 따라 해결못할 때도 있습니다.
	- ##### 소프트웨어로 즉 로직, 시스템구조로 해결하는 방법이 있습니다. 로직최적화로 IO를 최소화할 수 있습니다. 시스템구조를 수정해서 IO를 분산하여 한 서버의 부하를 낮출 수 있습니다.
- 동시에 10개의 컴퓨터에 라이브러리를 설치하는 일이 빈번히 발생합니다. 어떤 해결책이 있을까요?
	- ##### DevOps 툴을 사용시 다수 병렬 설치/설정이 가능합니다
	- ##### 라이브러리는 ... 가 있습니다(나중 추가)
